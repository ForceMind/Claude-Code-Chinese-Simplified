...
默认路径（等长）代价是覆盖率：中文比英文长的词条装不下（约 29%，存入
`dict/oversize.json` 备用）。**约 70% 界面中文 + 永久可用**，胜过 100% 但锁死旧版。

**`--full`（实验特性）**：把约束从「每条译文必须等长」放宽为「整个 JS 源码区总
长度不变」——短译文省下的字节汇成全局预算，容纳超长译文，装不下时按单条总
增量从贵到贱整条放弃（同一句话绝不会一半中文一半英文）。仍然零依赖、不解析
PE/Mach-O/ELF、模块表一个字节不动。实测 v2.1.220：`dict/oversize.json` 477
条超长词条中 387 条可命中，**全部放入**（安全条目省 9683 字节，超长条目仅需
5138 字节，净余 4545）。
...
10. **字符串分段器 + 比较谓词守卫**（`--full` 专属，默认路径同样受益）：手写 JS
    词法扫描把替换限定在 `'…'`/`"…"`/模板文本内（正确处理模板表达式嵌套、
    正则/除号歧义），并额外拒绝 `x.includes("…")`/`x==="…"`/`case "…":`
    这类**比较谓词**字符串——API/系统返回的是英文，谓词译成中文会让判断
    条件永假、功能静默失效（实证：某条历史词条曾让长上下文计费错误检测失效）。
11. **NTFS 长路径支持**：深层 nvm-windows / 企业环境安装路径很容易超过传统
    260 字符 `MAX_PATH` 限制，此前只有运行时验证一处做了绕过、其余文件操作
    在超长路径下会直接 `ENOENT` 且报错文案不提示真实原因。所有涉及定位/
    加锁/读写/改名的文件操作现在统一走 `\\?\` 扩展路径前缀（`src/winpath.js`
    + `src/wfs.js`），300+ 字符路径下实测正常。

**取舍说明**：跨进程加锁只以「记录的 pid 是否存活」为唯一依据，不设时间
兜底强制接管——2026-08-04 曾尝试加一条「锁放了 30 分钟就无条件允许接管」
来解决「pid 被系统复用给无关存活进程」这个理论边缘情况，结果引入了更严重
的反向风险：正常操作只要因杀软扫描/网络卡顿跑得比平时慢，就会被第二个
进程从仍在工作的第一个进程手里抢走活锁，导致两边并发写同一个 253MB
二进制。两轮独立审查都实测复现出这个回归，因此撤回——宁可留着「陈旧锁
偶尔抢不回来」这个烦人但有手动逃生口（报错提示删除锁文件）的缺口，也
不引入「活锁被误抢」这种无从挽回的风险。
