...
6. **语法级验证**：开发期从补丁产物中抽出内嵌 JS（16.5MB），esbuild 完整解析
   通过，与原版一致。
7. **运行时冒烟验证（最重要）**：补丁产物先落临时文件并**实际运行**
   （`--version` + `doctor`，用独立配置目录，不污染你的会话状态），
   跑通才覆盖你的 `claude.exe`；不通过就中止，你的文件一个字节都不会变。
   等长替换会扫到 Bun 运行时和第三方库的数据区，上面 1~6 条**都发现不了**
   这类破坏——实测有一条词条会命中 Bun 原生标识符表，导致 claude 启动即
   `TypeError: ptr.cancel is not a function`，正是这道验证把它挡住的
   （详见 DEVELOPMENT-NOTES §十一）。急用可加 `--no-verify` 跳过，不建议。
