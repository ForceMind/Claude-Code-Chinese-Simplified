...
7. **原生常量池守卫**：链接器会把短标识符与「以它结尾的长消息」**共享存储**
   ...
   「NUL 分隔的紧凑原生字符串表」并整体跳过（实测对 v2.1.220 跳过 38 处，
   界面文案零误伤）。词边界守卫只看紧邻字节，对这类完全无感。
8. **运行时冒烟验证（最重要）**：补丁产物先落临时文件并**实际运行**
   （`--version` + `doctor` + `mcp list`，用独立配置目录并关闭自动更新器，
   不污染你的会话状态），跑通才覆盖你的 `claude.exe`；不通过就中止，你的文件
   一个字节都不会变。上面 1~7 条**都发现不了**运行期才暴露的破坏——2026-08-03
   实测有一条词条会让 claude 启动即 `TypeError: ptr.cancel is not a function`，
   而字节断言、语法检查、`--version` 全部亮绿灯（`--version` 路径太浅，
   坏产物照样通过），正是这道验证把它挡住的（详见 DEVELOPMENT-NOTES §十一）。
   急用可加 `--no-verify` 跳过，**强烈不建议**。
